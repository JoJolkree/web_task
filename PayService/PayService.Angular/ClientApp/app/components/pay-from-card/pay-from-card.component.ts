...
export class PayFromCardComponent implements OnInit {
  ...
  save() {
      ...
      transaction.cardNumber = this.payData.CardNumber;
      transaction.monthExpired = this.payData.ExpirationDate;
      transaction.amount = parseInt(this.payData.Sum);
      transaction.comment = this.payData.Comment;
      transaction.email = this.payData.Email;
